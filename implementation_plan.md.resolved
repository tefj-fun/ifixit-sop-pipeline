...
*   **Role**: High-precision detection at runtime.
*   **Model Choice**: **State-of-the-Art (SOTA) Vision Transformer**.
    *   *Examples*: **Grounding DINO 1.5** (for open-set detection), **SAM 2** (Segment Anything 2 for masks), or **RT-DETR** (Real-Time DEtection TRansformer).
    *   *Why*: These models vastly outperform YOLO in zero-shot generalization and understanding complex prompts (like "the screw *inside* the hole").
*   **Training Strategy**:
    *   **Step 1: Pre-training (Transfer Learning)**: Train on the **ENTIRE** iFixit dataset.
    *   **Step 2: Fine-tuning**: Train on the user's specific data.
